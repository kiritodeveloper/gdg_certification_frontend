...
export function Select({
  ...
  className = '',
  ...props
}: FieldProps & { options: { value: string; label: string }[]; placeholder?: string } & InputHTMLAttributes<HTMLSelectElement>) {
  const inputId = props.id || (label || '').toLowerCase().replace(/\s+/g, '-');
  const needsPlaceholder = placeholder && !options.some((o) => o.value === props.value);
  return (
    <div className={`space-y-1.5 ${className}`}>
      {label && (
        <label htmlFor={inputId} className="block text-sm font-medium text-slate-700">
